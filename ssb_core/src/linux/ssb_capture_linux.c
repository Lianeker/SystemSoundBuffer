...
/* ------------------------------------------------------------- reproduccion
   ...
   Todavia no esta escrita. Devolver `ssb_err_platform` y decirlo es lo unico
   honesto: la interfaz ya sabe tratar ese error y avisa en vez de quedarse
   callada. */
...
void ssb_play_seek(ssb_play *p, double secs)
{
    (void)p;
    (void)secs;
}
